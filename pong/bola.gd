...
func _process(delta: float) -> void:
	movimentar_bola(delta) # Movimentar a bola
	colidir_com_paredes()
	

func resetar_bola() -> void: # reseta 
...
func movimentar_bola(delta: float) -> void: # movimenta a bola
	position += nova_direcao * velocidade_da_bola * delta

func colidir_com_paredes() -> void:
	if(position.y >= y_maximo or position.y <= y_minimo):
		if(position.x >= 0 and position.x <= 1200):
			nova_direcao.y *= -1 # inverte o movimento quando encosta na parede	
			$som_colisao_parede.play()


func _on_body_entered(body: Node2D) -> void:
	nova_direcao.x *= -1 # inverte o movimento quando encosta na parede
